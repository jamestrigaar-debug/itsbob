...
| `ITSBOB_OLLAMA_TIMEOUT` | seconds to wait for one local model call before giving up (default `20`; plain CPU inference on a modest laptop can genuinely take several seconds, well past the spec's optimistic <800ms) |
...
| A trace escalates with `"cloud response exceeded timeout"` but the reported latency is way past `cloud_timeout_seconds` (e.g. 7+ seconds against a 3s budget) | fixed as of this version — the timeout was previously only checked *after* the blocking cloud call finished, so it waited for the full slow response before discarding it late; it now genuinely cuts off at the deadline (a daemon thread carries the abandoned call, its result is never awaited) | if you still see a long wait before an escalation, that's the Gatekeeper's own local classification step being slow (see the next row), not the cloud timeout |
| Local classify latency is regularly 1–5 seconds, well past the spec's <800ms figure, sometimes with `(took Nms, over the 800ms target)` in the trace | plain CPU inference for a 1.5–3B model on a modest laptop is genuinely this slow — the <800ms figure in the original spec assumes a quantized model with reasonable throughput (a faster CPU, or GPU acceleration) | not a bug — the latency is now surfaced in the trace rather than hidden, and `ITSBOB_OLLAMA_TIMEOUT` (default 20s) has headroom for it; if it's too slow to be useful, try a smaller model (`ollama pull qwen2.5:0.5b`) or accept the heuristic classifier by not running Ollama at all |
